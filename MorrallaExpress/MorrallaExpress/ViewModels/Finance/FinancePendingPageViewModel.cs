...
{
    public class FinancePendingPageViewModel : ViewModelBase
    {
        ...
        public FinancePendingPageViewModel(INavigationService navigationService, IHttpService httpService)
        ...
        private async void RefreshListIntern()
        {
            ...
            History = History.Where(x => x.Status == Constants.DeliveredOrderStatus && x.Payout == null).ToList();
            History = History.Where(n => n.DeliveredDate >= StartDate && n.DeliveredDate <= EndDate.AddDays(1)).OrderByDescending(n => n.DeliveredDate).ToList();
            Orders.Clear();
